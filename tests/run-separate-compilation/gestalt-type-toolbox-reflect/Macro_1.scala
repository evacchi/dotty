...
object TypeToolbox {
  ...
  private def tpEqImpl[A, B](a: Type[A], b: Type[B])(implicit staging: Staging): Expr[Boolean] = {
  ...
  private def tpLEqImpl[A, B](a: Type[A], b: Type[B])(implicit staging: Staging): Expr[Boolean] = {
    import staging.reflection._
    val res = a.reflect.tpe <:< b.reflect.tpe
    res.toExpr
  }
  ...
  private def typeOfImpl(a: Expr[_], expected: Type[_])(implicit staging: Staging): Expr[Boolean] = {
    import staging.reflection._
  ...
  private def isCaseClassImpl(tp: Type[_])(implicit staging: Staging): Expr[Boolean] = {
    import staging.reflection._
  ...
  private def caseFieldsImpl(tp: Type[_])(implicit staging: Staging): Expr[List[String]] = {
    import staging.reflection._
  ...
  private def fieldInImpl(t: Type[_], mem: String)(implicit staging: Staging): Expr[String] = {
    import staging.reflection._
  ...
  private def fieldsInImpl(t: Type[_])(implicit staging: Staging): Expr[Seq[String]] = {
    import staging.reflection._
  ...
  private def companionImpl(t1: Type[_], t2: Type[_])(implicit staging: Staging): Expr[Boolean] = {
    import staging.reflection._
  ...
  private def companionNameImpl(tp: Type[_])(implicit staging: Staging): Expr[String] = {
    import staging.reflection._
  ...
  private implicit def listIsLiftable[T: Type: Liftable]: Liftable[List[T]] = new Liftable {
    def toExpr(list: List[T])(implicit staging: Staging): Expr[List[T]] = list match {
