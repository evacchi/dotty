...
object Lifters {
  ...
  implicit def ClassTagIsLiftable[T : Type](implicit ct: ClassTag[T]): Liftable[ClassTag[T]] = new Liftable[ClassTag[T]] {
    override def toExpr(ct: ClassTag[T])(implicit st: Staging): Expr[ClassTag[T]] = '(ClassTag(~ct.runtimeClass.toExpr))
  }

  implicit def ArrayIsLiftable[T : Type: ClassTag](implicit l: Liftable[T]): Liftable[Array[T]] = new Liftable[Array[T]] {
    override def toExpr(arr: Array[T])(implicit st: Staging): Expr[Array[T]] = '{
  ...
  implicit def IntArrayIsLiftable: Liftable[Array[Int]] = new Liftable[Array[Int]] {
    override def toExpr(arr: Array[Int])(implicit st: Staging): Expr[Array[Int]] = '{
      val array = new Array[Int](~arr.length.toExpr)
      ~initArray(arr, '(array))
    }
  }

  private def initArray[T : Liftable](arr: Array[T], array: Expr[Array[T]])(implicit st: Staging): Expr[Array[T]] = {
    UnrolledExpr.block(
