...
case class RingPV[U: Liftable](staRing: Ring[U], dynRing: Ring[Expr[U]])(implicit st: Staging) extends Ring[PV[U]] {
  type T = PV[U]
...
class RingIntPExpr(implicit st: Staging) extends RingPV(new RingInt, new RingIntExpr)

class RingIntOPExpr(implicit st: Staging) extends RingIntPExpr {
