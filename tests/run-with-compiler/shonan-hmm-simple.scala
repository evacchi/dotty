...
class RingIntExpr(implicit st: Staging) extends Ring[Expr[Int]] {
  val zero = '(0)
  val one  = '(1)
  val add  = (x, y) => '(~x + ~y)
  val sub  = (x, y) => '(~x - ~y)
...
case class Sta[T](x: T)(implicit st: Staging) extends PV[T] {
...
class RingPV[U: Liftable](u: Ring[U], eu: Ring[Expr[U]])(implicit st: Staging) extends Ring[PV[U]] {
...
object Complex {
  implicit def isLiftable[T: Type: Liftable]: Liftable[Complex[T]] = new Liftable[Complex[T]] {
    def toExpr(comp: Complex[T])(implicit st: Staging): Expr[Complex[T]] = '(Complex(~comp.re.toExpr, ~comp.im.toExpr))
  }
}
...
class ExprVecOps[T: Type](implicit st: Staging) extends VecOps[Expr[Int], Expr[T]] {
...
object Test {
  ...
  def main(args: Array[String]): Unit = {
    ...
    def blasStaticIntExpr(implicit st: Staging) = new Blas1(new RingIntExpr, new StaticVecOps)
    ...
    def blasExprIntExpr(implicit st: Staging) = new Blas1(new RingIntExpr, new ExprVecOps)
    ...
    def blasStaticIntPVExpr(implicit st: Staging) = new Blas1(new RingPV[Int](new RingInt, new RingIntExpr), new StaticVecOps)
    ...
    def blasExprIntPVExpr(implicit st: Staging) = new Blas1(new RingPV[Int](new RingInt, new RingIntExpr), new StaticVecOps)
    ...
    def blasExprComplexPVInt(implicit st: Staging) = new Blas1[Int, Complex[PV[Int]]](new RingComplex(new RingPV[Int](new RingInt, new RingIntExpr)), new StaticVecOps)
    ...
    def RingPVInt(implicit st: Staging) = new RingPV[Int](new RingInt, new RingIntExpr)
    // Staged loop of dot product on vectors of Int or Expr[Int]
    def dotIntOptExpr(implicit st: Staging) = new Blas1(RingPVInt, new StaticVecOps).dot
    // will generate the code '{ ((arr: scala.Array[scala.Int]) => arr.apply(1).+(arr.apply(3))) }
    def staticVec(implicit st: Staging) = Vec[Int, PV[Int]](5, i => Sta((i % 2)))
