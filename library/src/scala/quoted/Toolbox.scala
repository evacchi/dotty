...
trait Toolbox {
  ...
  protected def runImpl[T](code: Staging => Expr[T]): T // For Scala2 compat in ToolboxImpl

  def show[T](code: Staged[T]): String = runImpl(ctx => { implicit val c: Staging = ctx; code(ctx).show.toExpr })
}
